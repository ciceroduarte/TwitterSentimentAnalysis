...
public class TwitterAPI: APIProvider, TwitterUserProvider {
    ...
    public init(withTwitterKey twitterKey: String, session: URLSession) {
        self.twitterKey = twitterKey
        super.init(withSession: session)
    }

    public func fetchUser(byUsername username: String, completionHandler: @escaping (User?) -> Void) {
    ...
    public func fetchUserTimeline(withIdentifier userIdentifier: String, completion: @escaping ([Tweet]) -> Void) {
